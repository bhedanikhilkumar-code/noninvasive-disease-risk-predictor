...
export const validatePredictionInput = (payload) => {
  ...
  const allowedFields = new Set([
    'age', 'gender', 'bmi', 'bp_systolic', 'bp_diastolic', 'glucose',
    'heart_rate', 'smoking', 'alcohol', 'physical_activity', 'symptoms_text'
  ]);
  ...
  if (isFiniteNumber(payload.bp_systolic) && isFiniteNumber(payload.bp_diastolic) && payload.bp_diastolic >= payload.bp_systolic) {
    errors.push('bp_diastolic must be lower than bp_systolic.');
  }
  ...
  } else if (payload.symptoms_text.length > 2000) {
    errors.push('symptoms_text must be 2000 characters or fewer.');
  }
