...
accuracy, ROC-AUC, PR-AUC, precision, recall, F1, and Brier score in the model artifact, but these metrics
must not be interpreted as clinical evidence. A real deployment requires an approved, anonymized clinical
dataset, patient-level splitting, external validation, calibration review, and medical expert oversight. code.
